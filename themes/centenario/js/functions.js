(function ($) {
    ...
    $(document).ready(function ($) {
        ...
        $('.filter-form select').change(function () {
            var array = 'key=' + $(this).attr('name') + '&' + $('.filter-form').serialize();
            $.post('themes/centenario/ajax/filter.php', array, function (data) {
                if (data.type) {
                    $('select[name="type"]').html('<option value="">Tipo:</option>' + data.type);
                }
                if (data.finality) {
                    $('select[name="finality"]').html('<option value="">Finalidade:</option>' + data.finality);
                }
                if (data.district) {
                    $('select[name="district"]').html('<option value="">Bairro:</option>' + data.district);
                }
                if (data.bedrooms) {
                    $('select[name="bedrooms"]').html('<option value="">Quartos:</option>' + data.bedrooms);
                }
                if (data.min_price) {
                    $('select[name="min_price"]').html('<option value="">Valor mínimo:</option>' + data.min_price);
                }
                if (data.max_price) {
                    $('select[name="max_price"]').html('<option value="">Valor máximo:</option>' + data.max_price);
                }
            }, 'json');
        ...
        $("#btn_smt").on("click", function (event) {
            ...
            $("#btn_smt").addClass('btn-disabled');

            $.ajax({
                type: "POST",
                dataType: "json",
                url: "themes/centenario/ajax/contact.php",
                data: mydata,
                success: function (data) {

                    if (data["type"] == "error") {
                        $("#btn_smt").removeClass('btn-disabled');
