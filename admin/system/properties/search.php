<?php
    ...
    $read = new Read;
    $read->ExeRead("ws_properties", "WHERE realty_ref = :ref ORDER BY realty_status ASC, realty_date DESC LIMIT :limit OFFSET :offset", "ref={$ref}&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
    $realty = $read->getResult();
    /** se o resultado for nulo retorna uma mensagem */
    if (!$realty) {
        $pager->ReturnPage();
        Alert::set_flashdata('msg', '<i class="fa fa-exclamation fa-fw"></i> Não existe nenhum imóvel com o código ' . $ref, E_USER_NOTICE);
    }
...
<div class="container-fluid">
    ...
    <div class="col">
        <div class="row">
            <div class="col">
                <i class="fa fa-filter"></i> Resultado da pesquisa
            </div>
                    ...
                    foreach ($realty as $value) {
                        ...
                        <div class="card mb-4 box-shadow property-content item-<?= $realty_id; ?>"
                            ...
                            <div class="card-footer" style="text-align: center;">
                                <?php if (!$realty_status) { ?>

                                    <a class="mt-auto btn btn-sm btn-outline-warning"
                                       href="painel.php?exe=properties/index&realty=<?= $realty_id; ?>&action=active"
                                ...
                                <?php } else { ?>
                                    ...
                                    <a class="mt-auto btn btn-sm btn-outline-success"
                                       href="painel.php?exe=properties/index&realty=<?= $realty_id; ?>&action=inative"
